...
class BaseData:
    ...
    def validate(self) -> None:
        ...
        for v, services in self.virtual_machine_services.items():
            assert v in self.virtual_machines, f"{v} in virtual_machine_services is not a valid VM"
            for s in services:
                assert s in self.services, f"{s} in virtual_machine_services is not a valid service"

        # Validate service_base_costs
        for s in self.services:
            assert s in self.service_base_costs.keys(), f"Base cost for service {s} not defined"

        for s, cost in self.service_base_costs.items():
            assert s in self.services, f"{s} in service_base_costs is not a valid service"
        ...
        for v in self.virtual_machines:
            for t in self.time:
                assert (
                    v,
                    t,
                ) in self.virtual_machine_demand.keys(), f"No demand defined for VM {v} at time {t}"

        for (v, t), demand in self.virtual_machine_demand.items():
            assert v in self.virtual_machines, f"{v} in virtual_machine_demand is not a valid VM"
        ...
        for (s, instances) in self.max_service_instances.items():
            assert s in self.services, f"{s} in max_service_instances is not a valid service"
